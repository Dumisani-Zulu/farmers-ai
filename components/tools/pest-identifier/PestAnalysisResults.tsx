...
import { PestAnalysisResult } from '../../../ai/services/pest-identification-service';
...
export default function PestAnalysisResults({ results, onClose }: PestAnalysisResultsProps) {
  ...
  return (
    <ScrollView className="flex-1 bg-white p-4">
      <View className="flex-row items-center justify-between mb-6">
        <Text className="text-2xl font-bold text-gray-900">Pest Analysis Results</Text>
      ...
      <View className={`p-4 rounded-lg mb-4 ${results.pestDetected ? getSeverityBgColor(results.severity) : 'bg-green-50'}`}>
        <View className="flex-row items-center justify-between mb-2">
          <View className="flex-row items-center">
            <Ionicons 
              name={results.pestDetected ? "bug" : "checkmark-circle"} 
              size={24} 
              color={results.pestDetected ? "#EF4444" : "#10B981"} 
            />
            <Text className="text-lg font-semibold text-gray-800 ml-2">
              {results.pestDetected ? 'Pest Detected' : 'Plant Appears Healthy'}
            </Text>
          </View>
        ...
        {results.pestDetected && results.pestName && (
          <View>
      ...
      {results.indicators && results.indicators.length > 0 && (
        <View className="mb-4">
          {renderSection('Visible Indicators', results.indicators, 'eye-outline')}
        </View>
      )}
      ...
      <View className="bg-gray-50 p-4 rounded-lg mb-4">
        <Text className="text-sm text-gray-600 text-center">
          ⚠️ This analysis is for guidance only. For severe cases or confirmation, consult with a local agricultural expert or entomologist.
        </Text>
      </View>
    </ScrollView>
  );
